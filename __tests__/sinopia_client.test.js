...
describe("query", () => {
  it("successfully resolves the promise when response status is OK", async () => {
    ...
    expect(
      await sinopia_client.query("http://localhost:3000/groups")
  ...
  it("returns null if the api call fails", async () => {
    ...
    await expect(
      sinopia_client.query("http://localhost:3000/groups")
    ).rejects.toThrowError("Error parsing resource: ApiError")
  })
})
