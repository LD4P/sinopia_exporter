...
const initAndGetSavePath = (groupName, exportBasePath) => {
  ...
  if (dateTag) savePathString = [savePathString, dateTag].join("_")
...
const reportError = (errorObject, consoleErrMessage = null) => {
  ...
  if (consoleErrMessage) console.error(consoleErrMessage)
}
...
export const exportGroup = async (groupName, containingDir = "") => {
  ...
  let uri = [
    config.get("sinopia_api.basePath"),
    `/resource/?limit=${config.get("sinopia_api.query_limit")}&start=0&group=`,
    groupName,
  ].join("")
  ...
  let last_page = false
  do {
    await query(uri, { Accept: "application/json" })
      .then((groupResources) => {
        ...
        for (var i = 0; i < result_count; ++i) {
          ...
          try {
            ...
            reportError(
              err,
              `error saving resource ${groupName}/${resource.id} to ${savePathString} : ${err.stack}`
            )
            break
          }
        }
...
export const exportAllGroups = async () => {
  ...
  query(uri, { Accept: "application/json" })
